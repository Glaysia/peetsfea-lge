# peetsfea 0.3.8.0 — 사양서 (runner 의존 계약)

> peetsfea-runner가 의존 패키지 `peetsfea`에 요구하는 **계약 사양**이다. 현재 계약 버전은 **0.3.8.0**이다.
> 0.3.8.0의 핵심 변경은 SSW 형상 계약 정리, RX SSW 고정, RX no-SSW quarter-turn 변수 고정,
> 그리고 18차원 입력 설계공간이다. 기존 runner attach/solve/report 계약은 유지한다.
...
## 1. 버전 / 패키징
1. `peetsfea.__version__ == "0.3.8.0"`.
...
3. 진입점:
   `run_ssw_random_sample_reports_from_toml_text(candidate_toml_text, *, output_dir, seed, grpc_port, aedt_pid=None, ...)`.
